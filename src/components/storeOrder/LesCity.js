...
@inject('store')
@observer
class LesCity extends Component{
    constructor(props) {
        super(props);
    ...
    fetch=()=>{
        ...
        let url =`${Config.REPORT_URL_PREFIX}order-store/findCityLevel?`
            +`startDate=${this.state.startDate}&endDate=${this.state.endDate}&industry=${this.state.industry}`
            +`&invsort=${this.state.invsort}&price=${this.state.price}&matCode=${this.state.matnrData}`
            +`&currentUserId=${this.state.currentUserId}&cityLevel=${this.state.cityStatus}&ecology=${this.state.ecology}`;
    ...
    render(){
        ...
        let downloadUrl =`${Config.REPORT_URL_PREFIX}order-store/downLoadCityLevel?`
            +`startDate=${this.state.startDate}&endDate=${this.state.endDate}&industry=${this.state.industry}`
            +`&invsort=${this.state.invsort}&price=${this.state.price}&matCode=${this.state.matnrData}`
            +`&currentUserId=${this.state.currentUserId}&cityLevel=${this.state.cityStatus}&ecology=${this.state.ecology}`;
        return(
            <div className='allBorder'>
                    ...
                    <Col span={14}>
                        <ResponsiveContainer width='100%' height={320}>
                                <PieChart onMouseEnter={this.onPieEnter}>
                                    ...
                                    <Legend  content={<div style={{width:'100%',textAlign:'center'}}>线级城市销售比例(万元)</div>} />
                                </PieChart>
                        </ResponsiveContainer>
                    </Col>
                    <Col span={10}>
                ...
                {
                    salt.indexOf('W') !==-1 ?  <Button style={{marginTop:'20px'}} type="primary" onClick={this.showModal}>线级城市维护</Button>:''
                }
